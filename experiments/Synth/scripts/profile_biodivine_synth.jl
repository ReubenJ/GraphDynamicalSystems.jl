...
synth_params = Dict(
    ...
    "max_depth" => 6,
    "id" => res.ID,
    "n_trajectories" => collect(100:100:100),
...
function synth_one_biodivine(outer_params::AbstractDict{String,Any}, res::DataFrame)
    ...
    selected_trajs = rand(res.split_traj[1], n_trajectories)
...
function to_profile(synth_params, res)
    for params in dict_list(synth_params)
        synth_one_biodivine(params, res)
    end
end

@profview to_profile(synth_params, res)
@profview to_profile(synth_params, res)
